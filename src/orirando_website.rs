...
pub fn check_version(network: &NetworkSettings) -> Result<RandoVersion> {
    ...
    let resp = reqwest::blocking::get("https://bf.orirando.com/version/latest")
        .wrap_err("Error accessing bf.orirando.com")?;
    ...
    let html = resp
        .text()
        .wrap_err("Error getting text of bf.orirando.com")?;
...
pub fn download_dll(network: &NetworkSettings) -> Result<Vec<u8>> {
    ...
    let url = Url::from_str("https://bf.orirando.com/dll").expect("static URL is valid");
    info!(%url, "Downloading dll");

    let resp = reqwest::blocking::get(url).wrap_err("Error accessing bf.orirando.com")?;
